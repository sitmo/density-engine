...
def to_device(t1, t2, device):
    ...
    return t1.to(device, non_blocking=non_block), t2.to(device, non_block=non_block)
...
def compute_max_abs_diff(model, params, targets, quantile_levels):
    """
    Compute max absolute difference across all samples in a batch.
    
    Args:
        model: MDN 4.model
        params: Input parameters [B, 7]
        targets: Target quantiles [B, 512]
        quantile_levels: Reference quantile levels [512]
    
    Returns:
        Max absolute difference scalar
    """
    cdf_values = model.cdf(params, targets)  # [B, 512]
    batch_size = params.size(0)
    ref_quantiles = quantile_levels.unsqueeze(0).expand(batch_size, -1)
    
    # Compute absolute differences and take max across all [B, 512] samples
        ...
        while total_samples_processed < max_samples:
            try:
                ...
                if batch_idx % batches_per_lr_step == 0:
                    ...
                    # Print and log metrics every batches_per_lr_step batches
                    avg_loss = np.mean(batch_losses[-batches_per_lr_step:]) if len(batch_losses) >= batches_per_lr_step else np.mean(batch_losses)
                    avg_max_diff = np.mean(batch_max_diffs[-batches_per_lr_step:]) if len(batch_max_diffs) >= batches_per_lr_step else np.mean(batch_max_diffs)
                    ...
                    avg_outlier_loss = np.mean(outlier_batch_losses[-batches_per_lr_step:]) if len(outlier_batch_losses) >= batches_per_lr_step else (np.mean(outlier_batch_losses) if outlier_batch_losses else 0.0)
                ...
                # End of dataset, restart iterator
                train_iter = iter(train_loader)
                print("Restarting dataset iterator...")
